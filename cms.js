...
(function () {
  ...
  function renderChannelItems(items) {
    return (items || []).map(ch => {
      ...
      const external = ch.link && ch.link !== '#';
      return `
        <a href="${ch.link || '#'}" class="channel-item magnetic"${external ? ' target="_blank" rel="noopener"' : ''}>
          ${iconHtml}
